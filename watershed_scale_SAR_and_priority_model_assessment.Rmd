...
# Westslope Cutthroat Trout – Pacific (Oncorhynchus clarki lewisi)
...
sara_sciFilt<-sara %>% 
              filter(tolower(Scientific_Name) %in% tolower(c("Cottus aleuticus", "Oncorhynchus clarki lewisi", 
...
ggplot() + 
  ...
  geom_sf(data = zqpmUseFilt, 
          aes(fill = as.factor(number_distinct_SAR),
              colour = as.factor(number_distinct_SAR), size = 1.4)) + 
