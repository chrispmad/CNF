...
```{r filter_destination_waterbodies}
...
# Assign network ID to each priority wb. This helps us to later trim away downstream pieces from priority waterbodies that are further upstream than other priority waterbodies, in the same network.
downstream_courses_b = downstream_courses_b |> 
  dplyr::mutate(network_id = case_when(
    ...
    source_priority_wb %in% c("Kalamalka Lake","Okanagan Lake",
                              "Skaha Lake","Osoyoos Lake") ~ 3,
    T ~ 4
  ))
```

```{r subtract_overlapping_duplicate_geometries}
...
# Set up a new object to receive trimmed geometries
...
ggplot() + geom_sf(data = ds_by_wb_trimmed,
  ...
  labs(title = "Priority Waterbodies and their Downstream Courses",
       col = 'Waterbody Name')
...
### Priority Waterbodies and their Downstream Courses
...
### Number of Distinct SAR sp by Waterbody
```{r number_of_distinct_SAR_species_by_Priority_Waterbody}
sp_colors<-c( "#EFFDB1ff","#BDD793ff","#8BB174ff","#598B56ff","#276537ff")

ggplot() + 
  geom_sf(data = bc) +
  geom_sf(data = ds_by_wb_trimmed, col = 'grey', fill = 'grey') +
  geom_sf(data = zqpmUseFilt |> 
            dplyr::arrange(dplyr::desc(number_distinct_SAR_in_waterbody)), 
          aes(col = number_distinct_SAR_in_waterbody,
              fill = number_distinct_SAR_in_waterbody)) +
  ...
  labs(title = 'Number of Distinct SARA-listed Species overlapping Priority Waterbodies',
...
### Number of Distinct SAR Species Overlapping Downstream Courses
```{r number_of_distinct_SAR_species_by_Downstream_Courses}

# Find out which waterbodies had 2 or more SARA overlaps.
# wb_with_two_plus_sara = which(zqpmUseFilt$number_distinct_SAR_in_waterbody >= 2)
wb_for_labels = c("Flathead River","Lake Koocanusa","Moyie Lake")

ggplot() + 
  geom_sf(data = bc) +
  geom_sf(data = ds_by_wb_trimmed |> 
            dplyr::arrange(dplyr::desc(number_distinct_SAR_downstream)), 
          aes(col = number_distinct_SAR_downstream,
              fill = number_distinct_SAR_downstream)) +
  geom_sf(data = zqpmUseFilt, col = 'grey', fill = 'grey') +
  ggrepel::geom_text_repel(
    data = zqpmUseFilt |> 
      dplyr::filter(GNIS_NA %in% wb_for_labels),
    aes(geometry = geom,
        label = GNIS_NA),
    stat = 'sf_coordinates',
    force = 100,
  ...
  scale_colour_distiller(palette = 'Spectral') +
  scale_fill_distiller(palette = 'Spectral') +
  labs(title = 'Distinct SARA-listed Species by Waterbody Downstream Courses',
       subtitle = "(These are ADDITIONAL overlaps with downstream courses)",
       fill = "No. species at risk",
       col = 'No. species at risk')+
  ggthemes::theme_map() +
  coord_sf(xlim = sf::st_bbox(ws_high_priority)[c(1,3)],
...
```{r}
ds_by_wb_trimmed_wgs = sf::st_transform(ds_by_wb_trimmed, 4326)
zqpm_wgs = sf::st_transform(zqpmUseFilt, 4326)

my_pal = leaflet::colorNumeric(
  palette = 'Spectral',
  domain = zqpm_wgs$number_distinct_SAR_in_waterbody
)

l = leaflet() |> 
  addProviderTiles(providers$CartoDB) |> 
  addLayersControl(
    overlayGroups = unique(ds_by_wb_trimmed_wgs$source_priority_wb),
    options = layersControlOptions(collapsed = F)
  ) |> 
  addLegend(
    pal = my_pal,
    values = unique(ds_by_wb_trimmed_wgs$number_distinct_SAR_downstream)
  )
  
for(i in 1:nrow(ds_by_wb_trimmed_wgs)){
  l = l |> 
    addPolylines(
      data = ds_by_wb_trimmed_wgs[i,],
      label = ~paste0(source_priority_wb," downstream: ",number_distinct_SAR_downstream),
      color = ~my_pal(number_distinct_SAR_downstream),
      group = ~source_priority_wb
    ) |> 
    addPolylines(
      data = zqpm_wgs[i,],
      label = ~paste0(GNIS_NA,": ",number_distinct_SAR_in_waterbody),
      color = ~my_pal(number_distinct_SAR_in_waterbody),
      group = ~GNIS_NA
    )
}
