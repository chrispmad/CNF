...
>>>>>>> 4a25e8ae4c18a59c0b997f12629459f05fbac88a
```
...
#bcdc_search("Major Roads The Atlas of Canada Base Maps")
roadNetwork<-bcdc_query_geodata("7-5m-major-roads-the-atlas-of-canada-base-maps-for-bc") %>% 
  collect()
...
<<<<<<< HEAD
roadNetworkSection<- st_crop(roadNetwork, ws_Use_Overlap) 
roadNetworkSection

plot(roadNetworkSection$geometry)
...
```{r Road_Network_Plot}
ggplot()+
  geom_sf(data = st_crop(bc, ws_Use_Overlap), fill = "lightgrey", color = "darkgrey", alpha = 0.7)+
  geom_sf(data = ws_Sar, aes(fill = as.factor(number_distinct_SAR))) +
  scale_fill_manual(values =  sp_colors, na.value = "grey")+
  geom_sf(data= zqpm_Overlap, fill = "red", color="dark red") +
  geom_sf(data= ws_Use_Overlap, fill = "transparent", color = "purple", linewidth = 0.8) + 
  geom_sf(data = roadNetworkSection, color = "Black", linewidth = 1, alpha = 0.4)+
